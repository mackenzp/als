...
with open('Networks/networks.txt') as f:
   networks = f.read().splitlines()


###################### For First File ############################
filename="Networks/"+networks[0]
...
for episodes in range(1000):
   ...
   # Error in Mapping -200
   # Mapping successful +50
   # Mapping successful but area is more than previous one -100
   # Mapping successful and area is less than previous one +300
   alpha = 0.8;
   for i,j in zip(range(len(Q)),random_path):
         if (error_flag==0):
            if (area_new < area_old):
               area_old=area_new
               Q[i][j]=Q[i][j]+alpha*(50+300)
            else:
               Q[i][j]=Q[i][j]+alpha*(50-100)
         else:
            Q[i][j]=Q[i][j]+alpha*(-200)
print(Q)
...
for ckt in networks:
   ...
   # Modify Q matrix according to new nodes
   Q_old_len=len(Q)
   if (int(nodes[0]) > Q_old_len):
      for i in range(int(nodes[0])-Q_old_len):
         temp=[None]*(max_bit_error+1)
         Q.append(temp)
