...
***********************************************************************/
void writeGateError(Map_Man_t * pMan){
    Map_Node_t * pNode;
    Map_Super_t * pSuper;
    int nNodes, i,j, nFanins,nHamming;
    float Eg;
    FILE *fp;
    fp = fopen("gate_error.txt", "w");
    nNodes = (pMan->vMapObjs->nSize);
    for ( i = pMan->nInputs; i < nNodes; i++ )
    {
        pNode = pMan->vMapObjs->pArray[i];
        if(pNode->pCutBest[0]){
           nHamming=pNode->pCutBest[0]->M[0].pSuperBest->nHamming;
           if(nHamming!=0){
               nFanins=pNode->pCutBest[0]->M[0].pSuperBest->nFanins;
               Eg=nHamming/pow(2,nFanins);
               }
           else
               Eg=0;
           fprintf(fp,"%f\n",Eg);
               ...
               nHamming=pNode->pCutBest[1]->M[1].pSuperBest->nHamming;
              if(nHamming!=0){
                  nFanins=pNode->pCutBest[1]->M[1].pSuperBest->nFanins;
                  Eg=nHamming/pow(2,nFanins);
                  }
              else
                  Eg=0;
...
void Map_CalculateNumberOfNodes(Map_Man_t * p){
    Map_Node_t * pNode;
    Map_Cut_t * pCut;
    int nNodes, i;
    nNodes = p->vMapObjs->nSize;
    int nCuts[nNodes-(p->nInputs)];
    FILE *fp;
    //printf("\n%d\n",nNodes);
    ...
    for ( i = p->nInputs; i < nNodes; i++ )
    {
        pNode = p->vMapObjs->pArray[i];
        nCuts[i-(p->nInputs)]=0;
        for ( pCut = pNode->pCuts->pNext; pCut; pCut = pCut->pNext ){
             //printf("\nNode=%d,Cut=%d",i+1,(int)pow(2,pCut->nLeaves));
             if((int)pow(2,pCut->nLeaves) > nCuts[i-(p->nInputs)])
                nCuts[i-(p->nInputs)]=(int)pow(2,pCut->nLeaves);
    }
    fprintf(fp,"%d\n",nCuts[i-(p->nInputs)]);
}
   fclose(fp);
...
int Map_Evaluate(Map_Man_t * p ,int *bitErrors){
    Map_Node_t * pNode;
    Map_Cut_t * pCut;
    int nNodes, i,flag=0;
    ...
    for ( i = 0; i < nNodes; i++ )
    {
        ...
        for ( pCut = pNode->pCuts->pNext; pCut; pCut = pCut->pNext ){
             //printf("\nNode=%d,bitError=%d,Cut=%d",i,bitErrors[i],(int)pow(2,pCut->nLeaves));
             if(bitErrors[i] > (int)pow(2,pCut->nLeaves))
                flag=1;
             else
                flag=0;
    }
...
int Map_Mapping( Map_Man_t * p ,int *bitErrors)
{
    ...
    //if(!Map_Evaluate(p,bitErrors))
     // return 0;
    Map_MappingTruths( p ,bitErrors);
